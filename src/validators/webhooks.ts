...
export const webhookDeveloperIdSchema = z
  .string({
    error: (issue) =>
      issue.input === undefined ? 'developerId is required' : 'developerId must be a string',
  })
  .trim()
  .regex(
    developerIdPattern,
...
export const registerWebhookSchema = z
  .object({
    ...
    url: z
      .string({
        error: (issue) => (issue.input === undefined ? 'url is required' : 'url must be a string'),
      })
      .trim()
    ...
    events: z
      .array(z.enum(webhookManagementEvents), {
        error: (issue) => (issue.input === undefined ? 'events is required' : 'events must be an array'),
      })
