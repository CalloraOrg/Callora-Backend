...
jest.mock("../../src/services/sorobanBilling.js", () => {
  return {
    createSorobanRpcBillingClient: jest.fn().mockReturnValue({
      ...
      deductBalance: jest
        .fn()
        .mockResolvedValue({ txHash: "stellar-tx-hash-mock-123" }),
    }),
  };
});
  ...
  describe("POST /api/billing/deduct", () => {
    ...
    test("returns existing result for duplicate request_id", async () => {
      ...
      try {
        ...
        assert.equal(
          response2.body.stellarTxHash,
          response1.body.stellarTxHash,
        );
    ...
    test("handles Idempotency-Key header replays and conflicts", async () => {
      ...
      try {
        ...
        const reqHash = calculateRequestHash(
          "user_alice",
          requestBody,
          "POST",
          "/deduct",
    ...
    test("validates required fields", async () => {
      ...
      try {
        ...
        assert.ok(
          response2.body.message?.includes(
    ...
    test("returns 400 when developerId is null", async () => {
      const testDb = createTestDb();
      const token = createMockToken("user_null_dev");
      ...
      try {
        ...
        const response = await request(app)
          ...
          .set(createAuthHeaders(token))
          .send({
            requestId: "req_null_developer_id",
            developerId: null,
            apiId: "api_test",
            endpointId: "endpoint_test",
            apiKeyId: "key_test",
        ...
        assert.ok(response.body.message?.includes("developerId is required"));
  ...
  describe("GET /api/billing/request/:requestId", () => {
    test("returns billing request status", async () => {
      ...
      try {
        ...
        assert.equal(
          lookupResponse.body.usageEventId,
          createResponse.body.usageEventId,
        );
        assert.equal(
          lookupResponse.body.stellarTxHash,
          createResponse.body.stellarTxHash,
        );
    ...
    test("validates amount format and prevents negative values", async () => {
      ...
      try {
        ...
        for (const amount of invalidAmounts) {
          ...
          assert.ok(
            response.body.message?.includes("amountUsdc"),
            `Error should mention amountUsdc for: ${amount}`,
          );
        }
      } finally {
        await testDb.end();
